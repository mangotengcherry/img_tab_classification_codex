...
도메인 근거: FBM 불량 패턴 예시(세로선=bit-line, 가로선=word-line, 덩어리=cluster,
흩어진 점=random, edge ring)는 추가 논문의 이미지 예시를 참고했습니다
([fbm_domain_notes.md](fbm_domain_notes.md)). 모델 방법론은 그대로 fusion 실험에 둡니다.
...
|---|---:|---:|---:|
| 이미지 only | 0.700 | 0.533 | 0.373 |
| 전기 only | 1.000 | 0.517 | 0.517 |
| **fusion** | **0.972** | **0.983** | **0.956** |

- **fusion KPI 0.956 ≫ 단일 모달 최고 0.517** (차이 **+0.44**) → 합치는 게 확실히 이득.
- 이미지로 똑같은 구간: 이미지 **0.42** → 전기 **0.87** → fusion **0.96** (전기−이미지 **+0.45**).
- 전기 빼보기: fusion 정확도 0.975 → **0.446** (전기 기여 **+0.53**) → 이미지에만 의존하지 않음.
- 따라가기 비율 **0.94**(전기가 구한 18문제 중 17개를 fusion이 맞춤) → 쏠림(collapse) 없음.
